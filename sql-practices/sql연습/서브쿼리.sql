...
select*
from (select now() as n, sysdate() as b, 3 + 1 as c) s;    -- from 절에 서브쿼리를 줄때는 꼭 alias를 줘야한다.
 ...
 where a.emp_no = b.emp_no
   ...
   and b.dept_no = (select b.dept_no
					...
					where a.emp_no = b.emp_no
					 ...
					 and concat(a.first_name, '' , a.last_name) = 'Fai Bale') ;
-- 선생님
select b.dept_no											 -- 이걸로 fai bale가 근무하는 부서의 부서 ID를 알아낸다.
 from employees a, dept_emp b
where a.emp_no = b.emp_no
and b.to_date = '9999-01-01'
and concat(a.first_name, '' , a.last_name) = 'Fai Bale'; 
...
where a.emp_no = b.emp_no 
 ...
 and b.to_date = '9999-01-01'
 group by a.title
 order by avg(b.salary);   -- 이렇게 하면 안된다.

-- 2) 가장 적은 평균 급여 :soultion1
...
select a.title, avg(b.salary)
   ...
   where a.emp_no = b.emp_no 
	...
	and b.to_date = '9999-01-01'
 group by a.title
 order by avg(b.salary) asc  -- 오름 차순이라서 작은거 부터 보여준다
   limit 0,1;      			 -- limit 개수 제한두는거 0번째부터 1개 니깐 평균 연봉 제일 작은사람이 출력된다.
...
-- 실습문제3 : 현재 급여가 5000 이상인 직원의 이름을 출력하세요.
-- 다슬 50001
-- 둘리 50002
